...
- [x] `stmt` / `extension_stmt` baseline: Oracle `CREATE SEQUENCE`, `ALTER SEQUENCE`, `DROP SEQUENCE`, `MERGE`, `CALL`, `LOCK TABLE`, `EXPLAIN PLAN`, and `TRUNCATE TABLE` parser support for statements that are not represented by core schema contributor/query statements
