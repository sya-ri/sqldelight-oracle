...
- [ ] `select_stmt`: parser-backed coverage exists for representative `hierarchical_query_clause`, `qualify_clause`, flashback table syntax, pivot table syntax, and row-pattern table syntax; BNF drafts exist for representative `unpivot_clause` and remaining full row-pattern expressions; remaining work includes parser-backed verification for the draft forms, optimizer hints, `model_clause`, and full semantic validation
