...
- [ ] `select_stmt`: BNF drafts exist for `hierarchical_query_clause`, `qualify_clause`, representative `unpivot_clause`, and remaining full row-pattern expressions; parser-backed coverage exists for representative flashback table syntax, representative pivot table syntax, and representative row-pattern table syntax; remaining work includes parser-backed verification for the draft forms, optimizer hints, `model_clause`, and full semantic validation
- [ ] `table_or_subquery`: parser-backed coverage exists for database links, direct `ONLY` table references, partition/subpartition references, `SAMPLE`, `LATERAL`, `CONTAINERS`, `SHARDS`, legacy `THE (subquery)` table collections, `TABLE(collection)(+)`, `VALUES` table references, `GRAPHQL`, subquery restrictions, flashback table syntax, pivot table syntax, row-pattern table syntax, and modified external table references; BNF drafts exist for representative analytic view/hierarchy table references and inline external table references; remaining work includes parser-backed verification for the draft forms, analytic view/hierarchy semantic validation, join-to-one table expressions, unpivot column resolution, and full external table property validation
...
- [ ] `insert_stmt`: parser-backed coverage exists for remote targets, partition extension targets, `RETURNING`, `WAIT`, `BY NAME` / `BY POSITION`, DML error logging, and multi-table insert; remaining work includes direct-path hints, subquery insert variants, and `static_returning_clause`
