...
- [ ] `update_stmt`: parser-backed coverage exists for partition and remote targets, tuple subquery assignment, `WHERE CURRENT OF`, `RETURNING`, `WAIT`, direct-join `FROM`, and DML error logging; remaining work includes object-table and full `DML_table_expression_clause` coverage
