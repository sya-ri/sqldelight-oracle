...
- [x] `extension_stmt` baseline: Oracle `CREATE SEQUENCE`, `ALTER SEQUENCE`, `DROP SEQUENCE`, `MERGE`, `CALL`, and `LOCK TABLE` parser support for statements that are not represented by core schema contributor/query statements
