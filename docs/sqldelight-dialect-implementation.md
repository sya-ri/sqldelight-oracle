...
- [ ] `table_or_subquery`: parser-backed coverage exists for database links, direct and remote `ONLY` table references, partition/subpartition references, `SAMPLE`, `LATERAL`, `CONTAINERS`, `SHARDS`, legacy `THE (subquery)` table collections, `TABLE(collection)(+)`, `VALUES` table references, `GRAPHQL`, subquery restrictions, flashback table syntax, pivot/unpivot table syntax, row-pattern table syntax, and modified external table references; BNF drafts exist for representative analytic view/hierarchy table references and inline external table references; remaining work includes parser-backed verification for the draft forms, analytic view/hierarchy semantic validation, join-to-one table expressions, unpivot output-column resolution, and full external table property validation
