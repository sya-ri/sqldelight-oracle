...
- [ ] `insert_stmt`: parser-backed coverage exists for remote targets, partition extension targets, static `RETURNING`, `WAIT`, `BY NAME` / `BY POSITION`, `SET` insert clauses, DML error logging, and multi-table insert; remaining work includes direct-path hints and advanced subquery insert variants
