...
- [ ] `select_stmt`: parser-backed coverage exists for representative `hierarchical_query_clause`, `qualify_clause`, flashback table syntax, pivot/unpivot table syntax, and row-pattern table syntax; BNF drafts exist for remaining full row-pattern expressions; remaining work includes parser-backed verification for the draft forms, optimizer hints, `model_clause`, and full semantic validation
- [ ] `table_or_subquery`: parser-backed coverage exists for database links, direct, remote, and representative subquery `ONLY` table references, partition/subpartition references, `SAMPLE`, `LATERAL`, `CONTAINERS`, `SHARDS`, legacy `THE (subquery)` table collections, `TABLE(collection)(+)`, `VALUES` table references, `GRAPHQL`, subquery restrictions, flashback table syntax, pivot/unpivot table syntax, row-pattern table syntax, representative analytic view/hierarchy table references, and modified external table references; BNF drafts exist for inline external table references; remaining work includes parser-backed verification for the draft forms, analytic view/hierarchy semantic validation, join-to-one table expressions, unpivot output-column resolution, and full external table property validation
- [ ] `result_column`: Oracle aliases, `JSON`/`XML` returning clauses where expressions expose column names
- [ ] `function_expr`: parser-backed coverage exists for Oracle aggregate, analytic, `WITHIN GROUP`, `KEEP`, `OVER`, and aggregate `FILTER` clauses; remaining work includes function-specific `RETURNING`, unresolved overload semantics, and special argument separators
- [ ] `insert_stmt`: parser-backed coverage exists for remote targets, partition extension targets, `RETURNING`, `WAIT`, `BY NAME` / `BY POSITION`, `SET` insert clauses, DML error logging, and multi-table insert; remaining work includes direct-path hints, advanced subquery insert variants, and `static_returning_clause`
- [ ] `update_stmt`: parser-backed coverage exists for partition and remote targets, tuple subquery assignment, single-column and multi-column `DEFAULT` assignments, `VALUE(alias)` object-row assignment, `WHERE CURRENT OF`, `RETURNING`, `WAIT`, direct-join `FROM`, and DML error logging; remaining work includes object-table semantic validation and full `DML_table_expression_clause` coverage
- [ ] `delete_stmt`: parser-backed coverage exists for partition and remote targets, correlated `EXISTS` subqueries, `WHERE CURRENT OF`, `RETURNING`, and DML error logging; remaining work includes target-alias correlated subquery semantic validation, object-table support, and full `DML_table_expression_clause` coverage
