...
- [x] `extension_stmt` baseline: Oracle `CREATE SEQUENCE` and `DROP SEQUENCE` parser support for sequence DDL that is not represented by core schema contributor statements
