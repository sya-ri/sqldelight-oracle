...
- [x] `stmt` / `extension_stmt` baseline: Oracle `CREATE SEQUENCE`, `ALTER SEQUENCE`, `DROP SEQUENCE`, materialized/analytic/object/schema/graph/MLE/security/administrative and legacy drops, synonym/type/PLSQL/Java drops, role statements, `DROP USER`, `SET TRANSACTION`, `SET CONSTRAINTS`, `SET ROLE`, `SET USE DATA GRANTS ONLY`, `ANALYZE`, `ASSOCIATE STATISTICS`, `DISASSOCIATE STATISTICS`, `RENAME`, `COMMENT ON`, `PURGE`, `FLASHBACK`, `MERGE`, `CALL`, `LOCK TABLE`, `EXPLAIN PLAN`, `TRUNCATE TABLE`, and `TRUNCATE CLUSTER` parser support for statements that are not represented by core schema contributor/query statements
