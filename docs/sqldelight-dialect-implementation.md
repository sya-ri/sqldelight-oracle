...
- [ ] `select_stmt`: BNF drafts exist for `hierarchical_query_clause`, representative `flashback_query_clause`, representative `pivot_clause` / `unpivot_clause`, representative `row_pattern_clause`, and `qualify_clause`; remaining work includes parser-backed verification, optimizer hints, `model_clause`, and full semantic validation
- [ ] `table_or_subquery`: parser-backed coverage exists for `LATERAL`, `CONTAINERS`, `SHARDS`, subquery restrictions, and modified external table references; BNF drafts exist for database links, representative `ONLY` table references, legacy `THE (subquery)` table collections, representative analytic view/hierarchy table references, representative flashback table syntax, representative pivot/unpivot/row-pattern table syntax, and inline external table references; remaining work includes parser-backed verification for the draft forms, analytic view/hierarchy semantic validation, join-to-one table expressions, and full external table property validation
