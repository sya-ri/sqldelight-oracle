...
- [x] `extension_stmt` baseline: Oracle `CREATE SEQUENCE`, `ALTER SEQUENCE`, `DROP SEQUENCE`, `MERGE`, and `CALL` parser support for statements that are not represented by core schema contributor/query statements
