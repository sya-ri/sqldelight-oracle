...
public class ValidReturningClauseRule : Rule {
    ...
    override fun run(
        ...
        content
            .maskReturningCommentsAndQuotedTextPreservingOffsets()
            .returningStatements()
...
private fun String.maskReturningCommentsAndQuotedTextPreservingOffsets(): String {
    val chars = toCharArray()
    var index = 0
    while (index < chars.size) {
        index =
            when {
                startsWith("--", index) -> returningMaskRange(chars, index, skipReturningLineComment(index))
                startsWith("/*", index) -> returningMaskRange(chars, index, skipReturningBlockComment(index))
                chars[index] == '\'' -> returningMaskRange(chars, index, skipReturningQuotedString(index))
                else -> index + 1
            }
    }
    return String(chars)
...
private fun String.skipReturningLineComment(start: Int): Int = indexOf('\n', startIndex = start).let { if (it == -1) length else it }

private fun String.skipReturningBlockComment(start: Int): Int =
    indexOf("*/", startIndex = start + 2).let { if (it == -1) length else it + 2 }

private fun String.skipReturningQuotedString(start: Int): Int {
    var index = start + 1
    while (index < length) {
        if (this[index] == '\'') {
...
private fun returningMaskRange(
    chars: CharArray,
    start: Int,
    end: Int,
): Int {
