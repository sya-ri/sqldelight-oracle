    ...
    FunSpec({
        ...
        test("parses Oracle character single-row functions exactly") {
            val sql =
                ...
                SELECT CHR(65) AS chr_value,
                  ...
                  REPLACE(label, 'legacy', 'current') AS replaced_label,
                  REGEXP_COUNT(label, '[[:alpha:]]+', 1, 'i') AS alpha_group_count,
                  REGEXP_INSTR(label, '[[:alpha:]]+', 1, 2, 0, 'i', 0) AS second_alpha_group_position,
        ...
        test("parses Oracle query table expressions through SQLDelight environment exactly") {
            val sql =
                ...
                selectMatchRecognizePermutePattern:
                SELECT *
                FROM partitioned_orders MATCH_RECOGNIZE (
                  PATTERN (start_row PERMUTE(rising, falling+) steady?)
                ...
                selectMatchRecognizeBoundedPattern:
                SELECT *
                FROM partitioned_orders MATCH_RECOGNIZE (
                  PATTERN (start_row{2} rising{1,3}? falling{2,} steady{,3})
                  DEFINE start_row AS 1 = 1, rising AS 1 > 0, falling AS 0 < 1, steady AS 1 = 1
                ) bounded_matches;
        ...
        test("parses Oracle advanced alter table clauses through SQLDelight environment exactly") {
            val sql =
                ...
                ALTER TABLE alter_advanced_targets
                ADD CLUSTERING BY INTERLEAVED ORDER ((id, status), created_at)
                  YES ON LOAD
                  NO ON DATA MOVEMENT;
                ...
                ALTER TABLE alter_advanced_targets DROP CLUSTERING;
        ...
        test("parses Oracle insert error logging clauses through SQLDelight environment exactly") {
            val sql =
                ...
                insertTargetAliasReturning:
                INSERT INTO import_orders AS target (order_id, customer_name, order_total)
                VALUES (?, ?, ?)
                RETURNING target.order_id, target.order_total INTO ?, ?;
        ...
        test("parses Oracle update and delete partition extension clauses through SQLDelight environment exactly") {
            val sql =
                ...
                updateTargetAliasReturning:
                UPDATE partitioned_order_updates target
                SET order_total = target.order_total + ?
